...
namespace GadgetsOnline.Models
{
    ...
    // Registers Npgsql as the ADO.NET provider, connection factory, and
    // provider factory so that EF6 can resolve everything it needs at runtime
    // without relying on the old machine-level provider registration that was
    ...
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        // ---------------------------------------------------------------------
        // SaveChanges overrides — normalise all DateTime values to UTC before
        // persisting so that Npgsql's strict timestamptz handling is satisfied.
        // ---------------------------------------------------------------------
        ...
        {
            // Remove the default PluralizingTableNameConvention so that
            // explicit ToTable() names below are used without modification.
            modelBuilder.Conventions.Remove<PluralizingTableNameConvention>();

            // All tables live in the gadgetsonline_dbo schema on PostgreSQL.
            // Setting a default schema avoids having to repeat the schema name
            // on every individual ToTable() call, but each entity also carries
            // an explicit ToTable() for absolute clarity.
            ...
            // -----------------------------------------------------------------
            {
                ...
                // Relationship: Cart item requires one Product
