...
namespace GadgetsOnline.Models
{
    ...
    // EntityFramework6.Npgsql is used instead of the default SQL Server provider.
    // ---------------------------------------------------------------------------
    ...
    public class GadgetsOnlineEntities : DbContext
    {
        ...
        // Npgsql requires all DateTime values to carry DateTimeKind.Utc when writing
        // to timestamptz columns.  These overrides normalise any Unspecified/Local
        // values automatically before every save so callers need not be changed.
        // ---------------------------------------------------------------------------
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            // Relationships: HasMany(Products).WithRequired(Category).HasForeignKey(CategoryId)
            // -----------------------------------------------------------------------
            ...
            // Relationships: HasRequired(Product).WithMany().HasForeignKey(ProductId)
            // -----------------------------------------------------------------------
            ...
            // Relationships: HasMany(OrderDetails).WithRequired(Order).HasForeignKey(OrderId)
            // -----------------------------------------------------------------------
